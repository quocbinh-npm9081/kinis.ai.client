...
const CardPartnershipInvestComponent : React.FC<CardPartnershipInvestComponentProps> = ({title, listContent, contentButton}) => {
  return (
    <div className='flex flex-col justify-between h-full gap-3 px-7 py-6 md:px-8 md:py-7 lg:px-9 lg:py-8 xl:px-9 xl:py-8 2xl:px-10 2xl:py-9 2xl:gap-6 max-w-[630px] max-h-[479px] border border-[#FBFBFB] shadow-[0px_10px_10px_0px_rgba(0,0,0,0.1)] rounded-lg bg-white'>
        ...
        <div className="mt-auto flex justify-between items-center w-full">
            <button className='flex items-center text-nowrap justify-center gap-1 lg:gap-2 py-2 lg:py-4 text-black text-sm md:text-base lg:text-lg xl:text-xl 2xl:text-2xl italic'>Get more information</button>
            <button className='flex items-center text-nowrap justify-center gap-1 lg:gap-2 px-4 py-2 lg:px-6 lg:py-4 text-white bg-black hover:bg-black/80 transition-all duration-300 text-lg md:text-xl 2xl:text-2xl rounded-lg'>{contentButton} <Image src="/images/Vector.png" alt='arrow-right' width={24} height={24} className='w-auto h-auto'/></button>
        </div>
    </div>
